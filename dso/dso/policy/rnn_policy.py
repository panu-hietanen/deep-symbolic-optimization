        ...
        with tf.name_scope(f"controller{self.worker_id}"):
            ...
            def loop_fn(time, cell_output, cell_state, loop_state):
                ...
                if cell_output is None: # time == 0
                    ...
                    next_obs = state_manager.process_state(next_obs)
                    next_input = state_manager.get_tensor_input(next_obs)
                    next_obs_ta = obs_ta.write(time - 1, obs)
                    next_priors_ta = priors_ta.write(time - 1, prior)
                    finished = next_finished = tf.logical_or(next_finished, time >= max_length)
                    next_loop_state = (next_actions_ta, next_obs_ta, next_priors_ta,
                                       next_obs, next_prior, next_finished)
